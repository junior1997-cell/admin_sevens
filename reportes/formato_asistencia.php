...
// $trabajdor = $formatos_varios->datos_proyecto($_GET["id_proyecto"]);
...
if ($fecha_pago_obrero == 'semanal') {
  ...
  $dia_regular = 0;
  $weekday_regular = obtenerNombreDia($fechaInicio->format('Y-m-d'));
  $estado_regular = false;

  if ($weekday_regular == "Do") {
    $dia_regular = -1;
  } else {
    if ($weekday_regular == "Lu") {
      $dia_regular = -2;
    } else {
      if ($weekday_regular == "Ma") {
        $dia_regular = -3;
      } else {
        if ($weekday_regular == "Mi") {
          $dia_regular = -4;
        } else {
          if ($weekday_regular == "Ju") {
            $dia_regular = -5;
          } else {
            if ($weekday_regular == "Vi") {
              $dia_regular = -6;
            } else {
              if ($weekday_regular == "Sá") {
                $dia_regular = -7;
              }
            }
          }
  ...
  while ($cal_mes == false) {
    ...
    $val_fecha_f = new DateTime(format_a_m_d($fecha_f));
    $val_fecha_proyecto = $fechaFin;

    // var_dump($val_fecha_f);die();

    if ($val_fecha_f >= $val_fecha_proyecto) {
      $cal_mes = true;
    } else {
      $cal_mes = false;
    }
  ...
  while ($fechaInicio <= $fechaFin) {
    ...
    $hoja->setCellValue('D5', $proyecto['data']['nombre_proyecto']); //GET PROYECTO
    $hoja->setCellValue('D6', $proyecto['data']['ubicacion']); //GET UBICACIÓN
    $hoja->setCellValue('D7', $proyecto['data']['empresa']); //GET EMPRESA
  ...
  while ($fechaInicio <= $fechaFin) {
    ...
    $spreadsheet->getActiveSheet()->setTitle('Q' . $num_quincena++);
    $hojaActiva = $spreadsheet->setActiveSheetIndex($contador);
    $hojaActiva2 = $spreadsheet->getActiveSheetIndex($contador);
