...
// echo json_encode($full_epp_trabajador['datos_tr'],true);die();

//var_dump
$contador = 0;

foreach ($full_epp_trabajador['datos_tr'] as $key => $reg) {
  ...
  if (count($partes)) {
    $nombre = $partes[0] . ' ' . $reg['numero_documento'];
  } else {
    echo "No se pudo dividir.";
  }
  ...
  $totalCant =  count($reg['detalle_epp']);
  //var_dump($totalCant); die();

  // echo json_encode($totalCant,true);die();
  if ($totalCant == '0') {

    //echo json_encode('ddddddddddd',true);die();
    $ii = 0;
    for ($i = $k_i; $i < $k_i + 25; $i++) {
      $ii = $ii + 1;
      ...
      $spreadsheet->getActiveSheet()->getStyle('A' . $fila_3, ($i))->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN)->setColor(new Color('000000'));
      ...
      $spreadsheet->getActiveSheet()->getRowDimension($fila_3)->setRowHeight(30);
      $fila_3++;
    }
  } else {

    foreach ($reg['detalle_epp'] as $key => $val) {
