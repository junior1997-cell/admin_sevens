<?php
  ...
  if ($fecha_pago_obrero == 'semanal') {   
    ...
    foreach ($data_asistencia['data'] as $key => $reg) {
      ...
      $spreadsheet->getActiveSheet()->getStyle('A' . $fila_1 + 1 . ':w' . $fila_1 + 1)->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN)->setColor(new Color('000000'));      
      
      $hojaActiva->mergeCells('A' . $fila_1 . ':A' . $fila_1 + 1);          # unir columnas --
      ...
      $hojaActiva->mergeCells('E' . $fila_1 . ':E' . $fila_1 + 1);          # unir columnas ---
      $hojaActiva->setCellValue('E' . $fila_1, $reg['numero_documento']);   # DNI

      $hojaActiva->mergeCells('F' . $fila_1 . ':F' . $fila_1 + 1);          # unir columnas ---
      ...
      foreach ($reg['asistencia'] as $key3 => $val3) {
        ...
        $hojaActiva->setCellValue($dias_sem[$key3] . $fila_1 + 1, $val3['horas_extras_dia']);
        ...
        if (floatval($val3['horas_extras_dia']) > 0) {          
          $spreadsheet->getActiveSheet()->getStyle($dias_sem[$key3] . $fila_1 + 1)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('008000');
        } else {          
          $spreadsheet->getActiveSheet()->getStyle($dias_sem[$key3] . $fila_1 + 1)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FF0000');
        } 
      }      
      
      $hojaActiva->setCellValue('O' . $fila_1, $reg['total_hn']);                 # total HN
      $hojaActiva->setCellValue('O' . $fila_1 + 1, $reg['total_he']);             # total HN
      $hojaActiva->setCellValue('P' . $fila_1, $reg['total_dias_asistidos_hn']);  # Total dias
      $hojaActiva->setCellValue('P' . $fila_1 + 1, $reg['total_dias_asistidos_he']);  # Total dias

      $hojaActiva->mergeCells('Q' . $fila_1 . ':Q' . $fila_1 + 1);                # unir columnas ---
      $hojaActiva->setCellValue('Q' . $fila_1, $reg['sueldo_semanal']);           # Sueldo Semanal

      $hojaActiva->mergeCells('R' . $fila_1 . ':R' . $fila_1 + 1);                # unir columnas ---
      ...
      $hojaActiva->mergeCells('U' . $fila_1 . ':U' . $fila_1 + 1);                # unir columnas ---
      ...
      $total_q += floatval($reg['pago_quincenal_hn']) + floatval($reg['pago_quincenal_he']);
      $fila_1 += 2 ;
    }  
  ...
  header('Content-Disposition: attachment;filename="Asistencia_trabajador_HNE.xlsx"');
