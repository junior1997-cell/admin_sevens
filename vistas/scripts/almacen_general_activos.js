...
function init() {
  ...
  $("#lAlmacenGeneral_a").addClass("active");

  lista_de_items();
  tabla_principal('todos');

  // ══════════════════════════════════════ S E L E C T 2 ══════════════════════════════════════   Otro_Almacen
  lista_select2(`../ajax/almacen_general_activos.php?op=select2_proyect_almacen&tipo_transf=Proyecto&id_almacen_g=0`, '#proyecto_ag', null);
  lista_select2(`../ajax/almacen_general_activos.php?op=select2_proyect_almacen&tipo_transf=Otro_Almacen&id_almacen_g=0`, '#almacen_tup', null);
  lista_select2(`../ajax/almacen_general_activos.php?op=select2Productos`, '#producto_tup', null, '.cargando_producto_tup');
...
function reload_proyect_ag(pry) {
  ...
  if (idproyecto == null || idproyecto == '') {
    ...
    lista_select2(`../ajax/almacen_general_activos.php?op=select2ProductosComprados&idproyecto=${idproyecto}`, '#producto_ag', null, '.cargando_productos_ag');
...
function lista_de_items() {
  ...
  $.post("../ajax/almacen_general_activos.php?op=lista_de_categorias", function (e, status) {
...
function tabla_principal(id_categoria) {
  ...
  tabla_almacen_resumen = $("#tabla-almacen").dataTable({
    ...
    ajax: {
      url: `../ajax/almacen_general_activos.php?op=tabla_principal&id_categoria=${id_categoria}`,
      type: "get",
      dataType: "json",
      error: function (e) {
...
function guardar_y_editar_almacen(e) {
  ...
  $.ajax({
    url: "../ajax/almacen_general_activos.php?op=guardar_y_editar_almacen",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function mostrar(idalmacen_general) {
  ...
  $.post("../ajax/almacen_general_activos.php?op=mostrar", { 'idalmacen_general': idalmacen_general }, function (e, status) {
...
function eliminar(idproducto, nombre) {
  ...
  crud_eliminar_papelera(
    "../ajax/almacen_general_activos.php?op=desactivar",
    "../ajax/almacen_general_activos.php?op=eliminar",
    idproducto,
    "!Elija una opción¡",
...
function tabla_detalle(id_categoria, nombre, stock) {
  ...
  tabla_almacen_detalle = $("#tabla-detalle-almacen").dataTable({
    ...
    buttons: [
      ...
      { extend: 'copy', exportOptions: { columns: [0, 6,7,8, 2, 3, 4], }, text: `<i class="fas fa-copy" ></i>`, className: "px-3 btn btn-sm btn-outline-dark", footer: true, },
      { extend: 'excel', exportOptions: { columns: [0, 6,7,8, 2, 3, 4], }, title: `LISTA DE PRODUCTOS ALMACEN GENERAL ACTIVOS ${nombre_almacen_transf}`, text: `<i class="far fa-file-excel fa-lg" ></i>`, className: "px-3 btn btn-sm btn-outline-success", footer: true, },
    ],
    ajax: {
      url: `../ajax/almacen_general_activos.php?op=tabla_detalle&id_almacen=${id_categoria}&id_proyecto=${localStorage.getItem("nube_idproyecto")}&stock=${stock}`,
      type: "get",
      dataType: "json",
      error: function (e) {
        console.log(e.responseText); ver_errores(e);
...
function detalle_almacen_general(id_almacen_transf, idalmacen_general_resumen, nombre_producto) {
  ...
  tabla_detalle_almacen_general = $("#tabla_detalle_almacen_general").dataTable({
    ...
    ajax: {
      url: `../ajax/almacen_general_activos.php?op=tabla_detalle_almacen_general&id_almacen_transf=${id_almacen_transf}&idalmacen_general_resumen=${idalmacen_general_resumen}`,
      type: "get",
      dataType: "json",
      error: function (e) {
        console.log(e.responseText); ver_errores(e);
...
function guardar_y_editar_almacen_general(e) {
  ...
  $.ajax({
    url: "../ajax/almacen_general_activos.php?op=guardar_y_editar_almacen_general",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function select_tipo_transferencia(tipo) {
  ...
  if (tipo_transf == null || tipo_transf == 0) {
    ...
    lista_select2(`../ajax/almacen_general_activos.php?op=select2_proyect_almacen&tipo_transf=${tipo_transf}&id_almacen_g=${id_almacen_transf}`, '#name_alm_proyecto', null);
...
function listar_productos_transferencia() {
  ...
  $.post(`../ajax/almacen_general_activos.php?op=transferencia_a_proy_almacen&id_almacen=${id_almacen_transf}`, function (e, status) {
...
function add_producto_tup(data) {
  ...
  if (idproducto == null || idproducto == '' || idproducto === undefined) { } else {
    ...
    if ($(`#html_producto_tup div`).hasClass(`delete_multiple_${idproducto}`)) { // validamos si exte el producto agregado
      ...
      $.post(`../ajax/almacen_general_activos.php?op=marcas_x_producto`, { 'id_producto': idproducto}, function (e, status, jqXHR) {
...
function guardar_y_prod_id_tup(e) {
  ...
  $.ajax({
    url: "../ajax/almacen_general_activos.php?op=guardar_y_prod_id_tup",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function reload_producto_comprados_ag() { $('.comprado_todos_ag').html(`(comprado)`); lista_select2(`../ajax/almacen_general_activos.php?op=select2ProductosComprados&idproyecto=${idproyecto}`, '#producto_ag', null, '.cargando_productos_ag'); }

function obtener_dia_ingreso(datos) { $('#dia_ingreso_ag').val(extraer_dia_semana_completo($(datos).val())); }

function reload_producto_tup(){ lista_select2(`../ajax/almacen_general_activos.php?op=select2Productos`, '#producto_tup', null, '.cargando_producto_tup'); }
