...
function comprob_factura() {
  ...
  var precio_parcial = $("#total").val();

 // console.log('monto '+ monto +' cantidad '+ cantidad +' precio_unitario '+ precio_unitario);

  if ($("#tipo_comprobante").select2("val") =="Factura" && $("#total").val()!='' ) {
...
function listar() {
  ...
  tabla=$('#tabla-otro_servicio').dataTable({
    ...
    "ajax":{
        url: '../ajax/otros_servicios.php?op=listar',
        type : "get",
        dataType : "json",						
        error: function(e){
...
//console.log(extencion);
  ...
  if (extencion=='jpeg' || extencion=='jpg' || extencion=='png' || extencion=='webp') {
    $('#ver_fact_pdf').hide();
    $('#img-factura').show();
    $('#img-factura').attr("src", "../dist/img/comprob_otro_servicio/"+comprobante);

    $("#iddescargar").attr("href","../dist/img/comprob_otro_servicio/"+comprobante);
    ...
    $('#ver_fact_pdf').html('<iframe src="../dist/img/comprob_otro_servicio/'+comprobante+'" frameborder="0" scrolling="no" width="100%" height="350"></iframe>');
...
function guardaryeditar(e) {
  ...
  var formData = new FormData($("#form-activos-fijos")[0]);
 
  $.ajax({
    url: "../ajax/otros_servicios.php?op=guardaryeditar",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function mostrar(idotro_servicio) {
  limpiar();
  ...
  $.post("../ajax/otros_servicios.php?op=mostrar", { idotro_servicio: idotro_servicio }, function (data, status) {
    ...
    $("#idotro_servicio").val(data.idotro_servicio);
  ...
  if (data.comprobante != "") {
    ...
    $('#foto2_i').hide();
    $('#ver_pdf').show();
    $('#ver_pdf').html('<iframe src="../dist/img/comprob_otro_servicio/'+data.comprobante+'" frameborder="0" scrolling="no" width="100%" height="210"></iframe>');
...
function ver_datos(idotro_servicio) {

  $("#modal-ver-transporte").modal("show")

  $.post("../ajax/otros_servicios.php?op=verdatos", { idotro_servicio: idotro_servicio }, function (data, status) {
...
function total() {
  ...
  $("#total_monto").html("");
  $.post("../ajax/otros_servicios.php?op=total", { idproyecto: idproyecto }, function (data, status) {
...
function desactivar(idotro_servicio) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.post("../ajax/otros_servicios.php?op=desactivar", { idotro_servicio: idotro_servicio }, function (e) {
...
function activar(idotro_servicio) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.post("../ajax/otros_servicios.php?op=activar", { idotro_servicio: idotro_servicio }, function (e) {
...
$(function () {
  ...
  $("#form-activos-fijos").validate({
    rules: {
      forma_de_pago: { required: true },
      tipo_comprobante: { required: true },
      fecha_comprobante: { required: true },
      total:{required: true},            
      nombre_de_activo:{required: true},
      modelo_de_activo:{required: true}
      // terms: { required: true },
    },
    messages: {
      forma_de_pago: {
...
today = yyyy+'-'+mm+'-'+dd;
document.getElementById("fecha_comprobante").setAttribute("max", today);
