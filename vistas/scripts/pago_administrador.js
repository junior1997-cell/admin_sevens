...
function detalle_fechas_mes_trabajador(id_tabajador_x_proyecto, nombre_trabajador, fecha_inicial, fecha_hoy, fecha_final, sueldo_mensual, cuenta_bancaria, cant_dias_trabajando) {
  ...
  id_tabajador_x_proyecto_r = id_tabajador_x_proyecto; nombre_trabajador_r = nombre_trabajador; fecha_inicial_r = fecha_inicial;
  fecha_hoy_r = fecha_hoy; fecha_final_r = fecha_final; sueldo_mensual_r = sueldo_mensual; cuenta_bancaria_r = cuenta_bancaria; cant_dias_trabajando_r = cant_dias_trabajando;
  ...
  var monto_total = 0;  var monto_total_pagado = 0; var dias_regular_total = 0; var deposito_total = 0; var saldo_total = 0;
    ...
    $.post("../ajax/pago_administrador.php?op=mostrar_fechas_mes", { 'id_tabajador_x_proyecto': id_tabajador_x_proyecto }, function (data, status) {
      ...
      if (data.length === 0) {
        ...
        array_fechas_mes.forEach((element, indice) => {
          ...
          var monto_x_mes_e = 0; var nombre_mes_e = ""; var sueldo_mensual_e = 0; var recibos_x_honorarios_e = ""; var btn_tipo = ""; var suma_monto_depositado_e = 0;
          ...
          data.forEach(value => {
            if (value.fecha_inicial == format_a_m_d(element.fecha_i) && value.fecha_final == format_a_m_d(element.fecha_f) ) {
              ...
              if (value.recibos_x_honorarios == '' || value.recibos_x_honorarios == null) { btn_tipo = 'btn-outline-info'; } else { btn_tipo = 'btn-info'; }
            } 
          ...
          if (fechas_mes_estado) { 
            ...
            $('.data-fechas-mes').append(`<tr>
              ...
              <td>${format_d_m_a(fecha_final_e)}</td>
              ...
              <td>
                ...
                <button style="font-size: 14px;" class="btn btn-danger btn-sm">S/. ${formato_miles(suma_monto_depositado_e)}</button></div>
              </td>
              <td> S/. ${formato_miles(saldo_x_mes)}</td>
              <td> 
...
function modal_recibos_x_honorarios(idfechas_mes_pagos_administrador, id_tabajador_x_proyecto, fecha_inicial, fecha_final, mes_nombre, dias_mes, dias_regular, sueldo_mensual, monto_x_mes, recibos_x_honorarios, nombre_trabajador, cuenta_bancaria) {
  ...
  if (recibos_x_honorarios == '' || recibos_x_honorarios == null) {
    ...
    $('#descargar_rh').attr('href', `../dist/pago_administrador/recibos_x_honorarios/${recibos_x_honorarios}`);
    $('#descargar_rh').attr('download', `Recibo-por-honorario - ${mes_nombre} - ${nombre_trabajador}`); 
    $('#ver_completo').attr('href', `../dist/pago_administrador/recibos_x_honorarios/${recibos_x_honorarios}`);
    ...
    if ( extrae_extencion(recibos_x_honorarios) == "pdf" ) {
      $("#doc2_ver").html(`<iframe src="../dist/pago_administrador/recibos_x_honorarios/${recibos_x_honorarios}" frameborder="0" scrolling="no" width="100%" height="310"></iframe>`);
    } else {
      if ( extrae_extencion(recibos_x_honorarios) == "jpeg" || extrae_extencion(recibos_x_honorarios) == "jpg" || extrae_extencion(recibos_x_honorarios) == "jpe" ||
        ...
        $("#doc2_ver").html(`<img src="../dist/pago_administrador/recibos_x_honorarios/${recibos_x_honorarios}" alt="" width="50%" onerror="this.src='../dist/svg/error-404-x.svg';" >`); 
...
function desactivar_pago_x_mes(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.post("../ajax/pago_administrador.php?op=desactivar_ocupacion", { 'idpagos_x_mes_administrador': id }, function (e) {
...
function activar_pago_x_mes(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.post("../ajax/pago_administrador.php?op=activar_ocupacion", { 'idpagos_x_mes_administrador': id }, function (e) {
      ...
      if ( extrae_extencion(antiguopdf) == "doc") {
        ...
        if ( extrae_extencion(antiguopdf) == "docx" ) {
          ...
          if ( extrae_extencion(antiguopdf) == "pdf" ) {
            $("#doc"+id+"_ver").html(`<iframe src="../dist/pago_administrador/${carpeta}/${antiguopdf}" frameborder="0" scrolling="no" width="100%" height="310"></iframe>`);
                ...
                if ( extrae_extencion(antiguopdf) == "xlsx" ) {
                  ...
                  if ( extrae_extencion(antiguopdf) == "xlsm" ) {
                    ...
                    if (
                      ...
                      $("#doc"+id+"_ver").html(`<img src="../dist/pago_administrador/${carpeta}/${antiguopdf}" alt="" onerror="this.src='../dist/svg/error-404-x.svg';" width="50%" >`);
                      toastr.success('Documento vizualizado correctamente!!!');
                    } else {
