      ...
      switch ($_GET["op"]){
        ...
        case 'guardar_y_editar_mdo':
          ...
          if (empty($idmano_de_obra)){            
            $rspta=$mano_de_obra->insertar_mdo($idproyecto, $idproveedor, $ruc_proveedor, $fecha_inicial, $fecha_final, $monto, $descripcion);
            echo json_encode($rspta,true);
          } else {            
            $rspta=$mano_de_obra->editar_mdo($idmano_de_obra, $idproyecto, $idproveedor, $fecha_inicial, $fecha_final, $monto, $descripcion);            
            echo json_encode($rspta,true);
          }
            ...
            foreach ($rspta['data'] as $key => $reg) {
              ...
              $data[]=array(
                ...
                "2"=>'<div class="user-block">
                  ...
                  <span class="text-gray font-size-13px">'.$reg['tipo_documento'] .': '.$reg['ruc'].' </span>
                  </div>',
                "3"=>$reg['fecha_inicial'],  
                "4"=>$reg['fecha_final'],  
                "5"=>$reg['monto'],                 
