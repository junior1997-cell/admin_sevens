...
if (!isset($_SESSION["nombre"])) {
  ...
  if ($_SESSION['calendario'] == 1) {
    ...
    require_once "../modelos/Almacen_general_activos.php";

    $almacen_general_a = new Almacen_general_activos($_SESSION['idusuario']);
    ...
    switch ($_GET["op"]) {
      ...
      case 'guardar_y_editar_almacen':
        ...
        if (empty($idalmacen_general)) {
          ...
          $rspta = $almacen_general_a->insertar($nombre_almacen, $descripcion);
          echo json_encode($rspta, true);
        } else {
          $rspta = $almacen_general_a->editar($idalmacen_general, $nombre_almacen, $descripcion);
      ...
      case 'desactivar':
        $rspta = $almacen_general_a->desactivar($_GET["id_tabla"]);
        echo json_encode($rspta, true);
      break;

      case 'eliminar':
        $rspta = $almacen_general_a->eliminar($_GET["id_tabla"]);
        echo json_encode($rspta, true);
      break;

      case 'mostrar':
        $rspta = $almacen_general_a->mostrar($idalmacen_general);
        //Codificar el resultado utilizando json
        echo json_encode($rspta, true);
      break;

      case 'tabla_principal':
        $rspta = $almacen_general_a->tabla_principal($_GET["id_categoria"]);
      ...
      case 'lista_de_categorias':
        $rspta = $almacen_general_a->lista_de_categorias();
        echo json_encode($rspta, true);
      break;

      case 'tabla_detalle':

        $rspta = $almacen_general_a->tabla_detalle($_GET["id_proyecto"], $_GET["id_almacen"], $_GET['stock']);
      ...
      case 'tabla_detalle_almacen_general':
        ...
        $rspta = $almacen_general_a->tabla_detalle_almacen_general($_GET["id_almacen_transf"], $_GET["idalmacen_general_resumen"]);
      ...
      case 'guardar_y_editar_almacen_general':
        ...
        if (!empty($idalmacen_general_ag)) {
          ...
          $rspta = $almacen_general_a->insertar_alm_general(
      ...
      case 'marcas_x_producto':
        $rspta = $almacen_general_a->marcas_x_producto($_POST["id_producto"]);          
      ...
      case 'select2_proyect_almacen':
        ...
        $rspta = $almacen_general_a->select2_proyect_almacen($_GET['tipo_transf'], $_GET['id_almacen_g']);
      ...
      case 'select2ProductosComprados':
        ...
        $rspta = $almacen_general_a->select2_recursos_almacen($_GET["idproyecto"]);
        $cont = 1;
        $data = "";
      ...
      case 'select2Productos': 
        ...
        $rspta = $almacen_general_a->select2_productos_todos(); $cont = 1; $data = "";
      ...
      case 'guardar_transf_almacen_proyecto':
        ...
        $rspta = $almacen_general_a->guardar_transf_almacen_proyecto(
          $array_data_g,
      ...
      case 'guardar_y_prod_id_tup':
        $rspta = $almacen_general_a->guardar_y_prod_id_tup( $almacen_tup, $fecha_tup, $_POST["idproducto_tup"], $_POST["marca_tup"], $_POST["cantidad_tup"] );
        echo json_encode($rspta, true);           
      break;    

        //listar_productos de almacen para realizar transferencia
      case 'transferencia_a_proy_almacen':
        $rspta = $almacen_general_a->transferencia_a_proy_almacen($_GET["id_almacen"]);
        echo json_encode($rspta, true);
      break;
