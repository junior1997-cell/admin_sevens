    ...
    while ($reg = $rspta->fetch_object()) {
      ...
      if ($reg->estado_detraccion == "1") {
        ...
        $list_segun_estado_detracc =
          ...
          $c .
          ' btn-xs m-t-2px" onclick="listar_pagos(' .
          $reg->idcompra_proyecto .
          ',' .
          $reg->idproyecto .
          ',' .
      ...
      $data[] = [
        "0" =>
          $reg->estado == '1'
            ? '<button class="btn btn-info btn-sm" onclick="ver_detalle_compras(' .
              $reg->idcompra_proyecto .
              ...
              $reg->idcompra_proyecto .
              ')" data-toggle="tooltip" data-original-title="Anular Compra"><i class="far fa-trash-alt"></i></button>'
            : '<button class="btn btn-info btn-sm" onclick="ver_detalle_compras(' .
              $reg->idcompra_proyecto .
  ...
  case 'ver_detalle_compras':
    ...
    while ($reg = $rspta->fetch_object()) {
      ...
      echo '<tr class="filas">
        ...
        '</td>
                                    <td>' .
        number_format($reg->precio_venta, 2, '.', ',') .
        '</td>
                                    <td>' .
    ...
    echo '<tfoot>
      ...
      number_format($rspta2['subtotal_compras'], 2, '.', ',') .
      '</h5>
                            <h5 class="text-right igv_comp" style="font-weight: bold;">S/' .
      number_format($rspta2['igv_compras_proyect'], 2, '.', ',') .
      '</h5>
                            <b>
                                <h4 class="text-right total"  style="font-weight: bold;">S/' .
      number_format($rspta2['monto_total'], 2, '.', ',') .
      '</h4>
                            </b>
                    </tfoot>';
    ...
    while ($reg = $rspta->fetch_object()) {
      ...
      $datas[] = [
        "0" =>
          ...
          $reg->idproducto .
          ', \'' .
          htmlspecialchars($reg->nombre, ENT_QUOTES) .
          '\', \'' .
          $reg->nombre_medida .
          '\', \'' .
          $reg->nombre_color .
