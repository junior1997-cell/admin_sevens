      ...
      switch ($_GET["op"]) {
          ...
          foreach ($rspta as $key => $reg) {
            ...
            $data[] = [
              ...
              "4" => '<div class="user-block">'. 
                ...
                '<span class="description ml-0" ><b>Cel: </b><a class="text-body" href="tel:+51' . quitar_guion($reg['telefono']) . '" data-toggle="tooltip" data-original-title="Llamar al proveedor.">' . $reg['telefono'] . '</a> </span>'.
              '</div>',
              "5" => '<div class="user-block">'.
                '<span class="username ml-0"><p class="m-b-02rem" >' . $tipo_comprobante1 . '</p></span>
                  <span class="description ml-0" >Número: ' . $serie_comprobante . ' </span>
              </div>',
        ...
        case 'ver_detalle_compras_af_g':
          ...
          while ($reg = $rspta->fetch_object()) {
            ...
            empty($reg->ficha_tecnica) ? ($ficha = '<a ><i class="far fa-file-pdf fa-2x" style="color:#000000c4"></i></a>') : ($ficha = '<a target="_blank" href="../dist/docs/activos_fijos_general/ficha_tecnica_activos_fijos/' . $reg->ficha_tecnica . '"><i class="far fa-file-pdf fa-2x" style="color:#ff0000c4"></i></a>');
          ...
          echo '<tfoot>
            ...
            <th>
              ...
              <b> <h4 class="text-right total"  style="font-weight: bold;">S/' . number_format($rspta2['total'], 2, '.', ',') . '</h4></b>
          </tfoot>';
        ...
        case 'listarActivoscompra':
          ...
          while ($reg = $rspta->fetch_object()) {
            ...
            $datas[] = [
              "0" => '<button class="btn btn-warning" onclick="agregarDetalleCompraActivos(' . $reg->idproducto . ', \'' .  htmlspecialchars($reg->nombre, ENT_QUOTES) . '\', \'' . $reg->nombre_medida . '\', \'' . $reg->nombre_color . '\', \'' . $reg->precio_sin_igv . '\', \'' . $reg->igv . '\', \'' . $reg->precio_con_igv . '\', \'' .  $img_parametro . '\', \'' . $reg->ficha_tecnica . '\')" data-toggle="tooltip" data-original-title="Agregar Activo"><span class="fa fa-plus"></span></button>',
        ...
        case 'ver_detalle_compras_af_p':
          ...
          echo '<tfoot>
            ...
            <th> <h5 class="text-right subtotal"  style="font-weight: bold;">S/' . number_format($rspta2['subtotal'], 2, '.', ',') . '</h5>
              <h5 class="text-right igv_comp" style="font-weight: bold;">S/' . number_format($rspta2['igv'], 2, '.', ',') . '</h5>
              <b> <h4 class="text-right total"  style="font-weight: bold;">S/' . number_format($rspta2['total'], 2, '.', ',') . '</h4> </b>
            </tfoot>';
        break;
