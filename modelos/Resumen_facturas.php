...
//==============================================================
    ...
    if ($modulo == 'todos' || $modulo == 'otra_factura_proyecto') {    
      ...
      if ( !empty($fecha_1) && !empty($fecha_2) ) { $filtro_fecha = "AND otfp.fecha_emision BETWEEN '$fecha_1' AND '$fecha_2'"; } else if (!empty($fecha_1)) { $filtro_fecha = "AND otfp.fecha_emision = '$fecha_1'"; }else if (!empty($fecha_2)) { $filtro_fecha = "AND otfp.fecha_emision = '$fecha_2'"; }    

      if (empty($id_proveedor) ) {  $filtro_proveedor = ""; } else { $filtro_proveedor = "AND p.ruc = '$id_proveedor'"; }

      if ( empty($comprobante) ) { $filtro_comprobante = "AND otfp.tipo_comprobante IN ('Factura','Boleta','Nota de Crédito')"; } else { $filtro_comprobante = "AND otfp.tipo_comprobante = '$comprobante'"; }
      ...
      $filtro_proveedor $filtro_comprobante $filtro_fecha ORDER BY otfp.fecha_emision DESC;";
