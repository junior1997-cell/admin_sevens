...
class PensionV01
{
  ...
  {
    $desglese_array_detalle_pen = json_decode($array_detalle_pen, true);
    $sw = true;
    ...
    foreach ($desglese_array_detalle_pen as $indice => $key) {
      if (empty($key['iddetalle_pension'])) {
        ...
        $new_detalles_pension = ejecutarConsulta($sql_2);
        if ($new_detalles_pension['status'] == false) { return $new_detalles_pension; }

      } else {
        # editamos el registro existente
        $sql_3 = "UPDATE detalle_pension SET idservicio_pension='" . $key['idservicio_pension'] . "', fecha_pension='" .  $key['fecha_pension'] .
          "', dia_semana='" . $key['dia_semana'] . "', cantidad_platos='" . $key['cantidad_platos'] . "',	precio_plato='" . $key['precio_plato'] .
          "',	precio_parcial='" . $key['precio_parcial'] . "'	WHERE iddetalle_pension ='" . $key['iddetalle_pension'] . "';";

        $detalle_pension = ejecutarConsulta($sql_3);

        if ($detalle_pension['status'] == false) {
          return $detalle_pension;
        }
      }
    }
    $retorno = '';
    foreach ($desglose_array_semana_pen as $key => $value) {
      if (empty($value['idsemana_pension'])) {
        // insertamos un nuevo registro
        $sql_5 = "INSERT INTO `semana_pension`(`idservicio_pension`, `fecha_inicio`, `fecha_fin`, `numero_semana`, `precio_comida`, `cantidad_total_platos`, `adicional_descuento`,`total`,`presupuesto`, `descripcion`) 
				VALUES ('" . $value['idservicio_pension'] . "','" . $value['fecha_inicio'] . "','" . $value['fecha_fin'] . "','" . $value['numero_semana'] . "','" . $value['precio_comida'] . "','" . $value['cantidad_total_platos'] . "','" . $value['adicional_descuento'] . "','" . $value['total'] . "','" . $value['presupuesto'] . "','" . $value['descripcion'] . "')";
        $retorno = ejecutarConsulta($sql_5);
        if ($retorno['status'] == false) { return $retorno; }

      } else {
        # editamos el registro existente
        $sql6 = "UPDATE semana_pension SET idservicio_pension='" . $value['idservicio_pension'] . "',	fecha_inicio='" . $value['fecha_inicio'] . "',fecha_fin='" . $value['fecha_fin'] . "',	numero_semana='" . $value['numero_semana'] . "', precio_comida='" . $value['precio_comida'] . "',	cantidad_total_platos='" . $value['cantidad_total_platos'] . "',	adicional_descuento='" . $value['adicional_descuento'] . "', total='" . $value['total'] . "',	presupuesto='" . $value['presupuesto'] . "', descripcion='" . $value['descripcion'] . "' WHERE  idsemana_pension='" . $value['idsemana_pension'] . "';";
  ...
  public function ver_detalle_semana_dias($f1, $f2, $nube_idproyect, $id_pen)
  {
    ...
    if (!empty($servicio_pension['data'])) {
      foreach ($servicio_pension['data'] as $key => $value) {
        ...
        if ($datos_rangos_fechas['status'] == false) { return $datos_rangos_fechas; }

        $sql_3 = "SELECT idsemana_pension,precio_comida,cantidad_total_platos,adicional_descuento,total,presupuesto,descripcion 
        ...
        if ($rango_fecha_semana['status'] == false) { return $rango_fecha_semana; }

        if (empty($rango_fecha_semana['data'])) {
          $idsemana_pension = '';
          $precio_comida = '';
          $cantidad_total_platos = '';
          $adicional_descuento = '';
          ...
          $idsemana_pension = $rango_fecha_semana['data']['idsemana_pension'];
          $precio_comida = $rango_fecha_semana['data']['precio_comida'];
          $cantidad_total_platos = $rango_fecha_semana['data']['cantidad_total_platos'];
          $adicional_descuento = $rango_fecha_semana['data']['adicional_descuento'];
          $total = $rango_fecha_semana['data']['total'];
          $presupuesto = $rango_fecha_semana['data']['presupuesto'];
          $descripcion = $rango_fecha_semana['data']['descripcion'];
        }
  ...
  public function insertar_pension($idproyecto_p, $proveedor, $p_desayuno, $p_almuerzo, $p_cena, $descripcion_pension, $servicio_p)
  {
    ...
    if ($idpensionnew['status'] == false) { return $idpensionnew; }

    $id = $idpensionnew['data'];
    ...
    while ($num_elementos < count($servicio_p)) {
      if ($servicio_p[$num_elementos] == 'Desayuno') {
        $sql_servicio = "INSERT INTO servicio_pension(idpension,precio,nombre_servicio) VALUES ('$id','$p_desayuno','$servicio_p[$num_elementos]')";
        $sw = ejecutarConsulta($sql_servicio);
        if ($sw['status'] == false) { return $sw; }
      }

      if ($servicio_p[$num_elementos] == 'Almuerzo') {
        $sql_servicio = "INSERT INTO servicio_pension(idpension,precio,nombre_servicio) VALUES ('$id','$p_almuerzo','$servicio_p[$num_elementos]')";
        $sw = ejecutarConsulta($sql_servicio);
        if ($sw['status'] == false) { return $sw; }
      }

      if ($servicio_p[$num_elementos] == 'Cena') {
        $sql_servicio = "INSERT INTO servicio_pension(idpension,precio,nombre_servicio) VALUES ('$id','$p_cena','$servicio_p[$num_elementos]')";
        $sw = ejecutarConsulta($sql_servicio);
        if ($sw['status'] == false) { return $sw; }
      }
  ...
  public function editar_pension($idproyecto_p, $idpension, $proveedor, $p_desayuno, $p_almuerzo, $p_cena, $descripcion_pension, $servicio_p)
  {
    ...
    $update_pension = ejecutarConsulta($sql);
    if ($update_pension['status'] == false) { return $update_pension; }
    ...
    while ($num_elementos < count($servicio_p)) {
      if ($servicio_p[$num_elementos] == 'Desayuno') {
        ...
        if ($idbuscando_serv['status'] == false) { return $idbuscando_serv; }

        if (empty($idbuscando_serv['data']['idservicio_pension'])) {
          $sql_servicio = "INSERT INTO servicio_pension(idpension,precio,nombre_servicio) VALUES ('$idpension','$p_desayuno','Desayuno')";
          $sw = ejecutarConsulta($sql_servicio);
          if ($sw['status'] == false) { return $sw; }

        } else {
          $sql_servicio = "UPDATE servicio_pension SET precio='$p_desayuno' WHERE idservicio_pension='" . $idbuscando_serv['data']['idservicio_pension'] . "' ";
          $sw = ejecutarConsulta($sql_servicio);
          if ($sw['status'] == false) { return $sw; }
        }
      }
      ...
      if ($servicio_p[$num_elementos] == 'Almuerzo') {
        ...
        if (empty($idbuscando_serv['data']['idservicio_pension'])) {
          ...
          $sql_servicio = "UPDATE servicio_pension SET precio='$p_almuerzo' WHERE idservicio_pension='" . $idbuscando_serv['data']['idservicio_pension'] . "' ";
          $sw = ejecutarConsulta($sql_servicio);
          if ($sw['status'] == false) { return $sw; }
        }
      }
      ...
      if ($servicio_p[$num_elementos] == 'Cena') {
        ...
        if (empty($idbuscando_serv['data']['idservicio_pension'])) {
          ...
          $sql_servicio = "UPDATE servicio_pension SET precio='$p_cena' WHERE idservicio_pension='" . $idbuscando_serv['data']['idservicio_pension'] . "' ";
          $sw = ejecutarConsulta($sql_servicio);
          if ($sw['status'] == false) { return $sw; }
        }
      }
  ...
  public function total_x_pension($idpension)
  {
    ...
    foreach ($obt_servicio_pen['data'] as $key => $value) {
      ...
      $total_m = $total_m + $return_pension['data']['total'];
    }
  ...
  public function total_pension($idproyecto ) {
    $sql = "SELECT SUM(sp.total) as total
		...
		WHERE sp.idservicio_pension=se_p.idservicio_pension AND se_p.idpension=p.idpension AND p.idproyecto=py.idproyecto 
    ...
    if (
      $total_deposito['status'] == false) { return $total_deposito; 
    }else{
